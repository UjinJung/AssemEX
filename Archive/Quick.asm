...
QUICK	CLEAR	A	.466
	...
	ADD	STAADD
	STA	ENDADD
	.Pivot 계산 지금 A에 End 들어 있고 Start 더해서 2로 나눠줌
	ADD	STAADD
	DIV	#2
	STA	PIVIND
	
	.while(leftIndex <= rightIndex){
		.while(@leftIndex < @pivotIndex) leftIndex++;
		.while(@pivotIndex < @rightIndex) rightIndex--;
		...
		.if(StartAddress < rightIndex)
		.	Quick(StartAdd, rightIndex, returnAdd);
		.if(EndAddress > leftIndex)
		.	Quick(leftIndex, EndAddress, returnAdd);
...
.QUINUM	RESW	1
.STACK?
QUIIND	RESW	1	.ReturnAddress(3) | StartAddress(3)
	RESW	3
	RESW	3
	RESW	3
	RESW	3
